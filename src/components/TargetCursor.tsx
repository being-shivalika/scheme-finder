...
export interface TargetCursorProps {
  ...
  /** When set, cursor only appears while the pointer is inside this element. */
...
const TargetCursor = ({
  ...
  const inScopeRef = useRef(!scopeRef);
  ...
  const moveCursor = useCallback((x: number, y: number) => {
    if (!cursorRef.current || !inScopeRef.current) return;
  ...
  useEffect(() => {
    ...
    const setScopedActive = (active: boolean) => {
      inScopeRef.current = active;
      if (active) {
    ...
    const createSpinTimeline = () => {
      ...
      if (scopeRef) spinTl.current.pause();
    };
    createSpinTimeline();

    const tickerFn = () => {
      if (!inScopeRef.current) return;
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      if (scopeEl) {
        ...
        if (inside !== inScopeRef.current) setScopedActive(inside);
        if (!inside) return;
      }
    ...
    const scrollHandler = () => {
      if (!inScopeRef.current || !activeTarget || !cursorRef.current) return;
    ...
    const mouseDownHandler = () => {
      if (!inScopeRef.current || !dotRef.current) return;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (!directTarget || !(directTarget instanceof Element)) return;

      const scopeEl = scopeRef?.current;
      if (scopeEl && !scopeEl.contains(directTarget)) return;
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && inScopeRef.current && cursorRef.current && spinTl.current) {
